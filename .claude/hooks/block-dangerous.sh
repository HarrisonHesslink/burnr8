...
# Checks the full command string including subshells and bash -c wrappers.
INPUT=$(cat)
...
# Catches direct invocation and bash -c / sh -c wrappers
...
# Block destructive commands wrapped in bash -c / sh -c
if echo "$COMMAND" | grep -qE '(bash|sh)\s+-c\s+' && \
   echo "$COMMAND" | grep -qE '(rm\s+-rf|git\s+push.*--force|git\s+reset\s+--hard)'; then
  echo "Blocked: destructive command in subshell. Run manually if needed." >&2
  exit 2
fi
