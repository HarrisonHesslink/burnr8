...
- **v30 mutate pattern**: All mutate calls must use request objects via `build_mutate_request()` helper. `client.get_type()` returns a proto **instance**, not a class — direct kwargs were removed in SDK v30.
- **`campaign.start_date` / `campaign.end_date`**: Removed in newer API versions. Do not include in GAQL queries.
